...
def trainer(phase, train_loader,
    ...
    for epoch in range(max_epochs):
        ...
        # train for one epoch
        loss_training = train(phase, train_loader, model, criterion, optimizer, epoch + 1, train=True, debug=debug)

        # evaluate on validation set
        loss_validation = train(phase, val_loader, model, criterion, optimizer, epoch + 1, train=False, debug=debug)
...
def train(phase, dataloader, model, criterion, optimizer, epoch, train=False, debug=False):
        ...
        for i, batch in t:
            ...
            if phase == "spotting":
                ...
                for i in range(len(video_features)):
                    video_features[i] = video_features[i].cuda()
                labels = cls_labels.cuda() # Bx2 (0, 7)
                # labels = labels[:, 1].long().cuda()
                # compute output
                output = model(video_features, feats, positions) # Bx18
                ...
                # compute output
                output = model(video_features, feats, positions)
                # hand written NLL criterion
                loss = criterion(output, cls_labels)
            else:
                NotImplementedError()
...
def validate_spotting(dataloader, model, model_name):
    ...
    all_outputs = []
    with tqdm(enumerate(dataloader), total=len(dataloader)) as t:
        for i, ((feats, caption), lengths, mask, caption_or, cap_id, cls_labels, video_features, positions) in t:
            # measure data loading time
            data_time.update(time.time() - end)
            feats = feats.cuda()
            for i in range(len(video_features)):
                video_features[i] = video_features[i].cuda()
            labels = (cls_labels > 0) * 1.0
    ...
    for i in range(1, dataloader.dataset.num_classes+1):
        AP.append(average_precision_score(all_labels, all_outputs[:, i]))
    ...
    with torch.no_grad():
        with tqdm(enumerate(dataloader), total=len(dataloader)) as t:
            for i, ((feats, caption), lengths, mask, caption_or, cap_id, cls_labels, video_features, positions) in t:
                ...
                total_predictions += cls_labels.size(0)
                correct_predictions += (predicted.cpu().detach() == cls_labels).sum().item()
...
def test_spotting(dataloader, model, model_name, save_predictions=True, NMS_window=30, NMS_threshold=0.5):
    ...
    with tqdm(enumerate(dataloader), total=len(dataloader)) as t:
        for i, (game_ID, feat_half1, feat_half2, label_half1, label_half2, video_feature1, video_feature2, position1, position2) in t:
            data_time.update(time.time() - end)
            # Batch size of 1
            game_ID = game_ID[0]
            ...
            video_features2 = []
            for i in range(BS):
                video_features1.append(video_feature1.squeeze(0).clone().cuda())
            for i in range(BS):
                video_features2.append(video_feature2.squeeze(0).clone().cuda())

            # TODO: position1 and position2 are not used in the model
            for b in range(int(np.ceil(len(feat_half1)/BS))):
                start_frame = BS*b
                if BS * (b+1) < len(feat_half1):
                    end_frame = BS*(b+1)
                else:
                    end_frame = len(feat_half1)
                    video_features1 = video_features1[:end_frame-start_frame]

                feat = feat_half1[start_frame:end_frame].cuda()
                # if len(feat_half1) < len(position1):
                #     position1 = position1[:len(feat_half1)]
                # elif len(feat_half1) > len(position1):
                #     position1.extend([[[]] for _ in range(len(feat_half1)-len(position1))])
                pos1 = position1[start_frame:end_frame]
                output = model(video_features1, feat, pos1)
                output = torch.nn.functional.softmax(output, dim=1) # Bx18
                #output = torch.stack([output[:, 0], 1-output[:, 0]], dim=1)
            ...
            for b in range(int(np.ceil(len(feat_half2)/BS))):
                ...
                if BS * (b+1) < len(feat_half2):
                    ...
                    end_frame = len(feat_half2)
                    video_features2 = video_features2[:end_frame-start_frame]

                feat = feat_half2[start_frame:end_frame].cuda()
                # if len(feat_half2) < len(position2):
                #     position2 = position2[:len(feat_half2)]
                # elif len(feat_half2) > len(position2):
                #     position2.extend([[[]] for _ in range(len(feat_half2)-len(position2))])
                pos2 = position2[start_frame:end_frame]
                output = model(video_features2, feat, pos2)
...
def validate_captioning(dataloader, model, model_name):
    ...
    with tqdm(dataloader) as t:
        for (feats, caption), lengths, mask, caption_or, cap_id, cls_labels, video_features, positions in t:
            # measure data loading time
            data_time.update(time.time() - end)
            feats = feats.cuda()
            for i in range(len(video_features)):
            ...
            output = [dataloader.dataset.detokenize(list(model.sample(video_features[idx], feats[idx], positions[idx]).detach().cpu())) for idx in range(feats.shape[0])]
...
def test_captioning(dataloader, model, model_name, output_filename = "results_dense_captioning.json", input_filename="results_spotting.json"):
    ...
    with tqdm(dataloader) as t:
        for video_features, feats, game_id, cap_id, positions in t:
            # measure data loading time
            data_time.update(time.time() - end)
            feats = feats.cuda()
            for i in range(len(video_features)):
    ...
    medium = evaluate_dvc(SoccerNet_path=dataloader.dataset.path, Predictions_path=output_results, split=dataloader.dataset.split, version=dataloader.dataset.version, prediction_file=output_filename, window_size=15, include_SODA=False)
    ...
    results = {**{f"{k}_tight" : v for k, v in tight.items()}, **{f"{k}_loose" : v for k, v in loose.items()}, **{f"{k}_medium" : v for k, v in medium.items()}}
    return results