...
from dataset import SoccerNetDataset,SoccerNetClipsTesting, collate_fn_gpt, collate_fn_clip
...
def main(args):
    ...
    if not args.test_only:
        dataset_Train = SoccerNetDataset(path=args.SoccerNet_path, pos_path=args.Position_path, features=args.features, split=args.split_train, version=args.version, framerate=args.framerate, 
                                     caption_window_size=args.window_size_caption, spot_window_size=args.window_size_spotting, insert_empty_caption=True)
    ...
    if not args.test_only:
        ...
        #criterion = partial(sigmoid_focal_loss, alpha=0.25, reduction="mean")
        ...
        trainer("spotting", train_loader, val_loader, val_metric_loader, 
                ...
                max_epochs=15, evaluation_frequency=15, wandb_use=args.wandb, debug=args.debug) 
    ...
    for split in args.split_test:
        dataset_Test  = SoccerNetClipsTesting(path=args.SoccerNet_path, pos_path=args.Position_path, features=args.features, split=[split], version=args.version, framerate=args.framerate, window_size=args.window_size_spotting)

        test_loader = torch.utils.data.DataLoader(dataset_Test,
            batch_size=1, shuffle=False,
            num_workers=1, pin_memory=True, collate_fn=collate_fn_clip)
