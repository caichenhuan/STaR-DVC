...
这份代码仓库现在已经整理成适合公开发布的版本。阅读时建议先把它看成一个四阶段流水线，而不是若干独立脚本。
...
2. `run_base.sh`
3. `main.py`
4. `classifying.py` / `captioning.py` / `spotting.py`
5. `dataset.py`
...
- `--stage full`：按 `classifying -> caption -> spotting -> dvc` 执行完整流程
- `--stage classifying`：只跑事件分类预训练
...
- `classifying` 产出分类编码器权重
- `caption` 产出字幕模型权重
- `spotting` 产出事件定位结果
- `dvc` 使用 `caption` 权重和 `spotting` 输出生成最终 dense captions

如果启用 `--pretrain`，`spotting` 会自动尝试加载
`models/<model_name>/classifying/model.pth.tar` 作为编码器初始化。
...
- 入口：`classifying.py`
- 模型：`model.py` 中的 `Video2Classifcation`
- 作用：先把视频编码器训到能做足球事件分类，供后续定位阶段加载
