...
from dataset import SoccerNetDataset, PredictionCaptions, collate_fn_padd, collate_fn_gpt, collate_fn_pred
from model_gpt import Video2CaptionGPT
...
def main(args):
    ...
    if not args.test_only:
        ...
        dataset_Valid = SoccerNetDataset(path=args.SoccerNet_path, pos_path=args.Position_path, features=args.features, split=args.split_valid, version=args.version, framerate=args.framerate, 
                                         caption_window_size=args.window_size_caption, spot_window_size=args.window_size_spotting, insert_empty_caption=False)
        dataset_Valid_metric  = dataset_Valid # SoccerNetCaptions(path=args.SoccerNet_path, features=args.features, split=args.split_valid, version=args.version, framerate=args.framerate, 
                                                # caption_window_size=args.window_size_caption, spot_window_size=args.window_size_spotting, insert_empty_caption=False)
    ...
    if not args.test_only:
        ...
        trainer("caption", train_loader, val_loader, val_metric_loader, 
                ...
                max_epochs=args.max_epochs, evaluation_frequency=args.evaluation_frequency, wandb_use=args.wandb, debug=args.debug)
...
def dvc(args):
    ...
    for split in args.split_test:
        ...
        dataset_Test  = PredictionCaptions(SoccerNetPath=args.SoccerNet_path, pos_path=args.Position_path, PredictionPath=PredictionPath, features=args.features, split=[split], version=args.version, framerate=args.framerate, window_size=args.window_size_caption)

        test_loader = torch.utils.data.DataLoader(dataset_Test,
            batch_size=args.batch_size, shuffle=False,
            num_workers=args.max_num_worker, pin_memory=True, collate_fn=collate_fn_pred)
        ...
        logging.info(f'| Bleu_1_medium: {results["Bleu_1_medium"]}')
        logging.info(f'| Bleu_2_medium: {results["Bleu_2_medium"]}')
        logging.info(f'| Bleu_3_medium: {results["Bleu_3_medium"]}')
        logging.info(f'| Bleu_4_medium: {results["Bleu_4_medium"]}')
        logging.info(f'| METEOR_medium: {results["METEOR_medium"]}')
        logging.info(f'| ROUGE_L_medium: {results["ROUGE_L_medium"]}')
        logging.info(f'| CIDEr_medium: {results["CIDEr_medium"]}')
        logging.info(f'| Recall_medium: {results["Recall_medium"]}')
        logging.info(f'| Precision_medium: {results["Precision_medium"]}')
